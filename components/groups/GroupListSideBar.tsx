...
const Component: FC<ComponentProps> = async ({ user }) => {
    ...
    const hoverCss = theme === "dark" ? "hover:text-black hover:bg-zinc-200" : "hover:text-white hover:bg-zinc-700";

    return (
        <aside className="w-2/12 h-screen p-4">
            <div className="flex flex-col justify-between h-full">
                <div>
                    <h2 className="text-lg font-semibold mb-4">{user?.username || "Unknown"} Groups</h2>
                    <ul>
                        {groups.map((group) => (
